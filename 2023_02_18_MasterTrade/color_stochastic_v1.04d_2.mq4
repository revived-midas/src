...
#property indicator_color3 Red //UPPER
#property indicator_color4 Blue //LOWER
#property indicator_color5 Red //UPTREND
#property indicator_color6 Blue //DOWNTREND
   ...
   if (showArrows)
      {
         ...
         for (i=0; i<WindowBarsPerChart() ;i++)
            {
               if (KFull[i]>overBought && KFull[i+1]<overBought) DrawArrow(i,"down");
              // org if (KFull[i]<overSold   && KFull[i+1]>overSold)   DrawArrow(i,"up");
               if (KFull[i]>overSold   && KFull[i+1]<overSold)   DrawArrow(i,"up");
            }
      }            
   if (alertsOn)
         {
      ...
      if (type=="up")
         {
            ...
            if (alertArrowOn) doAlert(" Down Trend Alert ");
         }
      else
         {
            ...
            if (alertArrowOn) doAlert(" Up Trend Alert ");
         }
}
void DeleteArrows()
{
...
void doAlert(string doWhat)
{
   ...
   if (Period()== 43200)   TimePeriod = "MONTHLY";
      ...
      if (previousAlert != doWhat || previousTime != Time[0]) {
          ...
          message =  StringConcatenate(Symbol()," at ", TimePeriod, " Color Stochastic : ",doWhat);
             if (alertsMessage && Bars>alertBar) {Alert(message);alertBar = Bars;}
             if (alertArrowOn && Bars>alertBar) {PlaySound("alert2.wav");alertBar = Bars;}
             if (alertsSound && Bars>alertBar)  {PlaySound("alert2.wav");alertBar = Bars;}
             if (alertsEmail && Bars>alertBar)   {SendMail(StringConcatenate(Symbol()," Color Stochastic "),message);alertBar = Bars;}
      }        
}
