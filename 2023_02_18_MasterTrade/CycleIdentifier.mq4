...
int Price1BuyB=1,Price2BuyB=1;
//---
double Price1SellA=0.0,Price2SellA=0.0;
int Price1SellB=0,Price2SellB=0;
//---
...
int start()
  {
   ...
   if(counted_bars==0) position-=(FilterStrengthRSI+4);
//----
   int rnglength=250;
   ...
   for(int pos=position; pos>=0; pos--)
     {
      ...
      if(UseFilterSMAorRSI==2)
         ZL1[pos]=ZeroLag(iRSI(NULL,0,14,(ENUM_APPLIED_PRICE)CyclePrice,FilterStrengthRSI),FilterStrengthRSI,pos);
     ...
     /* PrintFormat("Bars = %d | counted_bars = %d | pos = %d | BarNumber = %d",Bars,counted_bars,pos,BarNumber);
      PrintFormat("srange = %f | range = %f",srange,range);
      PrintFormat("CyclePrice = %f | ZL1[%d] = %f",CyclePrice,pos,ZL1[pos]);
      PrintFormat("SwitchC = %d",SwitchC);
      PrintFormat("Strength = %f | SweepA = %f | Price1BuyA = %f | Price1SellA = %f", Strength,SweepA,Price1BuyA,Price1SellA);
      */
